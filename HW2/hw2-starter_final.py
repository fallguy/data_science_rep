"""
  ...
  In general the code is clean and easy to read. Consider
    adding a bit more white space in-between groups of code.
    Also add comments when it may not be obvious what blocks of code do.
"""
...
def dist(x, y):
    ...
    # Nice condensed way of doing this! 
    # Note that technically this would not work for numeric string inputs
...
def nearest_athletes(athlete, athletes, k = 1):
    ...
    # Keep in mind to add white space inbetween lines!
    # The technique here works, but it will be slow since you are 
    #   creating so many extra lists every time you you find the nearest neighbor!
    # Consider trying the key= property of sorted()

    # print(athlete)
    ath_dist = []
    for ath in athletes:
        
        # What about athletes with the same name?
        #   You can alternatively compare dictionaries directly to see if they're equal!
        if ath['name'] != athlete['name']:
            # This works -- you can also convert the test athlete into a list at the top
            #   of the function, to avoid having to remake the test athlete's list
...
print("FINAL ACCURACY at k=50: 27.85%")
